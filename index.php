...
$scores = [
  'rasp' => [1 => [750, 'Platinum'], 2 => [850, 'Diamond']]
];
...
foreach ($_GET as $key => $value) {
  ...
  if ($scores[$key]) {
    if ($i > count($scores[$key])) {
      break;
    }
    foreach ($scores[$key] as $key2 => $value) {
      #echo $key2;
      #echo $scores[$key][$i][0];
      echo $scores[$key][$i][0];
      if ($_GET[$key] >= $scores[$key][$i][0] && $_GET[$key] < $scores[$key][$i + 1][0]) {
        echo 'tutaj';
        echo $scores[$key][$i][1];
        break;
      }
      if ($i == count($scores[$key])) {
        echo 'xddd';
        if ($_GET[$key] >= $scores[$key][$i + 1][0]) {
          echo $scores[$key][$i + 1][1];
          echo 'kupa';
          break;
        }
      }
